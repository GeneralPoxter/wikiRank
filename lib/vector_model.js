...
function idf(corpus, term) {
    return Math.log10(corpus.documents.length / (1 + (corpus.docFrequencies.get(term) || 0)));
}
...
export function getDocumentRanks(wikidocs, query) {
    ...
    const queryWeights = vectorize(parseDocument({ text: query }));
    ...
    const ranks = new Map();
    corpus.documents.forEach(document =>
        ranks.set(document, sim(queryWeights, weights.get(document)))
    );
